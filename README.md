...
V1을 만들면서 MVC 한 프로젝트 안에 화면, API 역할, 실시간 처리 로직이 같이 들어가 있었습니다. 기능을 추가할수록 프론트와 서버 코드가 같은 배포 단위에 묶이는 점이 불편했습니다.

V2에서는 화면을 React로 분리하고, 조회와 실시간 처리는 ASP.NET Core API 서버로 옮겼습니다. 기존 최근 고장 관제 기능은 유지하면서 누적 고장 이력을 조건별로 조회하는 Search 화면도 추가했습니다.
